...
impl Client {
    ...
    /* pub async fn offer(&mut self, target: &str) -> Result<(), anyhow::Error> {
        if let Some(ref mut write) = self.write {
            let offer = SignalMessage::Offer(SessionDescription {
                sender: self.name.clone(),
                target: target.to_string(),
                description: generate_description(32),
            });
            let data = serde_json::to_string::<SignalMessage>(&offer).unwrap();
            write.send(Message::Text(data.into())).await?;
        } else {
    ...
    pub async fn answer(&mut self, target: &str) -> Result<(), anyhow::Error> {
        if let Some(ref mut write) = self.write {
            let answer = SignalMessage::Answer(SessionDescription {
                sender: self.name.clone(),
                target: target.to_string(),
                description: generate_description(32),
            });
    ...
    /* async fn _receive(
        ...
        while let Some(msg) = reader.next().await {
            match msg {
                Ok(Message::Text(text)) => {
                    let data = text.to_string();
                    match serde_json::from_str::<SignalMessage>(&data) {
                        Err(_) => {},
                        Ok(msg) => {
                            match msg {
                                SignalMessage::Register(_) => {},
                                SignalMessage::Answer(sdp) | SignalMessage::Offer(sdp) => {
                                    _ = sender.send(sdp);
                                },
                            }
                        }
